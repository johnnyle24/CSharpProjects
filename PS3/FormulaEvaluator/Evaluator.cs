...
namespace FormulaEvaluator
{
    ...
    public static class Evaluator
    {
        ...
        public static double Evaluate(String exp, Func<string, double> variableEvaluator)
        {
            ...
            double result = 0;
            Stack<double> values = new Stack<double>(); //Stack for the doubles
            Stack<string> operators = new Stack<string>(); //Stack for all the operands
            ...
            for (int i = 0; i < substrings.Length; i++)
            {
                ...
                if (double.TryParse(substrings[i], out value))
                {
                    if (operators.Count != 0 && operators.Peek().Equals("*"))
                    {
                        ...
                        double popv = values.Pop();
                        operators.Pop();
                        double evalv = popv * value;
                        values.Push(evalv);
                    }
                ...
                {
                    double variable_value = variableEvaluator(substrings[i]);
                    if (operators.Count != 0 && operators.Peek().Equals("*"))
                    {
                        if (values.Count == 0)
                        {
                        ...
                        double popv = values.Pop();
                        operators.Pop();
                        double evalv = popv * variable_value;
                        values.Push(evalv);
                    }
                    else if (operators.Count != 0 && operators.Peek().Equals("/"))
                    {
                ...
                else if (substrings[i].Equals("+") | substrings[i].Equals("-"))
                {
                    ...
                    if (operators.Count != 0 && operators.Peek().Equals("+"))
                    {
                        ...
                        double popv = values.Pop();
                        double popv2 = values.Pop();
                        operators.Pop();
                        double evalv = popv + popv2;
                        values.Push(evalv);
                    }
                    else if (operators.Count != 0 && operators.Peek().Equals("-"))
                    {
